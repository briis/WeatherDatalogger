-- Migration: 20260707_dedupe_pressure_role_overlap.sql
-- The davis_station_pressure_mb/davis_sea_level_pressure_mb/davis_pressure_trend*/
-- davis_wet_bulb_c/davis_delta_t_c/davis_air_density_kgm3 columns in
-- combined_realtime and history_charting exist so the Davis receiver's own
-- on-board BME280 reading can sit alongside a *different* device's `pressure`
-- role (historically Tempest) without being merged. If `pressure` is
-- reassigned to `davis` (station_roles.role = 'pressure' -> 'davis') — i.e.
-- the same station now supplies both `pressure` and `temp_humidity` — the
-- `pr` and `th` (view) / roles.pressure_type and roles.temp_humidity_type
-- (event) references resolve to the exact same row, so these davis_*
-- columns become a byte-for-byte duplicate of the non-prefixed ones.
--
-- Fix: null out the davis_* variants whenever the `pressure` and
-- `temp_humidity` roles point at the same station_type, so the pressure
-- data is only shown once. No schema change — this only touches the view
-- and the aggregation event.

USE weatherdatalogger;

-- ─────────────────────────────────────────────────────────────────────────────
-- View: combined_realtime — adds the `ro` cross join (a single boolean flag:
-- does `pressure` point at the same station_type as `temp_humidity`) and
-- wraps the 9 davis_* pressure/wet-bulb/delta-T/air-density columns in a
-- CASE that nulls them out when it does. Otherwise identical to
-- migrations/20260707_add_wind_beaufort.sql.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Event: evt_aggregate_history_charting — same dedup, using
-- MAX(roles.pressure_type) = MAX(roles.temp_humidity_type) instead of a bare
-- column comparison, since `roles` columns must stay wrapped in an aggregate
-- function to satisfy ONLY_FULL_GROUP_BY under the query's GROUP BY window_start.
-- ─────────────────────────────────────────────────────────────────────────────
