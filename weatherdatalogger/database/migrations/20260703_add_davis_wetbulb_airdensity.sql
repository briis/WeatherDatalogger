-- Migration: 20260703_add_davis_wetbulb_airdensity.sql
-- The Davis receiver's BME280 now also drives on-device wet bulb
-- temperature (iterative bisection, same as tempest_datalogger.py's
-- _wet_bulb_c()), delta T, and air density — needs temp/humidity (already
-- RF-decoded) plus station pressure (BME280), so it piggybacks on the same
-- comfort-metrics computation in davis-vantage-receiver.yaml.
--
-- wet_bulb_c/delta_t_c/air_density_kgm3 already exist on realtime/history
-- (shared with Tempest's `pressure` role) — no new column needed there,
-- it's just populated for davis rows too now. Only history_charting needs
-- new davis_-prefixed columns, following the same pattern as the previous
-- two Davis migrations today.

USE weatherdatalogger;

ALTER TABLE history_charting
    ADD COLUMN IF NOT EXISTS davis_wet_bulb_c       FLOAT NULL AFTER davis_sea_level_pressure_trend,
    ADD COLUMN IF NOT EXISTS davis_delta_t_c        FLOAT NULL AFTER davis_wet_bulb_c,
    ADD COLUMN IF NOT EXISTS davis_air_density_kgm3 FLOAT NULL AFTER davis_delta_t_c;

-- ─────────────────────────────────────────────────────────────────────────────
-- View: combined_realtime — adds davis_wet_bulb_c/davis_delta_t_c/
-- davis_air_density_kgm3, sourced from the existing temp_humidity(davis)
-- join (th). Otherwise identical to migrations/20260703_add_davis_pressure_trend.sql.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Event: evt_aggregate_history_charting — adds the same 3 davis_-prefixed
