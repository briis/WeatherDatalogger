-- Migration: 20260702_davis_primary_combined_view.sql
-- Switch combined_realtime (and history_charting) to source weather data from
-- the Davis Vantage station instead of Tempest, now that Davis is field-tested
-- and reliable. Exceptions, because the Davis ISS has no sensor for them:
--   - UV / solar radiation           -> stays on Tempest (no sensor fitted)
--   - Pressure (station + sea level) -> stays on Tempest (Davis has no barometer)
--   - Lightning                      -> stays on Tempest (Davis has no detector)
--   - Wet bulb / delta T             -> stays on Tempest (not computed by Davis)
--   - Air density                    -> stays on Tempest (not computed by Davis)
--   - Battery voltage                -> stays on Tempest; Davis only reports a
--                                        battery_low boolean, exposed as a new
--                                        davis_battery_low column instead.
-- Air quality (PM/AQI) is unchanged — still sourced from AirLink.
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Table: history_charting — add the Davis low-battery flag
-- ─────────────────────────────────────────────────────────────────────────────
ALTER TABLE history_charting
    ADD COLUMN IF NOT EXISTS davis_battery_low BOOLEAN NULL AFTER battery_volts;

-- ─────────────────────────────────────────────────────────────────────────────
-- Event: evt_aggregate_history_charting — same Davis/Tempest/AirLink split as
-- the view above.
-- ─────────────────────────────────────────────────────────────────────────────
