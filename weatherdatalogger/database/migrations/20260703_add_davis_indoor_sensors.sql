-- Migration: 20260703_add_davis_indoor_sensors.sql
-- The Davis receiver now has a BME280 soldered to the ESP32, co-located with
-- the receiver itself (not the outdoor ISS). It reports indoor temperature,
-- indoor humidity, and its own barometer reading.
--
-- station_pressure_mb already exists on realtime/history (previously always
-- NULL for davis rows, since Davis had no barometer) — no new column needed
-- there, it's just populated now. indoor_temperature_c/indoor_humidity_pct
-- are new. All three are sourced from the existing temp_humidity(davis) role
-- join, same as davis_battery_low already is — no new role required.

USE weatherdatalogger;

ALTER TABLE realtime
    ADD COLUMN IF NOT EXISTS indoor_temperature_c FLOAT NULL COMMENT 'Davis receiver BME280' AFTER battery_low,
    ADD COLUMN IF NOT EXISTS indoor_humidity_pct   FLOAT NULL COMMENT 'Davis receiver BME280' AFTER indoor_temperature_c;

ALTER TABLE history
    ADD COLUMN IF NOT EXISTS indoor_temperature_c FLOAT NULL AFTER battery_low,
    ADD COLUMN IF NOT EXISTS indoor_humidity_pct   FLOAT NULL AFTER indoor_temperature_c;

ALTER TABLE history_charting
    ADD COLUMN IF NOT EXISTS indoor_temperature_c      FLOAT NULL AFTER davis_battery_low,
    ADD COLUMN IF NOT EXISTS indoor_humidity_pct        FLOAT NULL AFTER indoor_temperature_c,
    ADD COLUMN IF NOT EXISTS davis_station_pressure_mb  FLOAT NULL AFTER indoor_humidity_pct;

-- ─────────────────────────────────────────────────────────────────────────────
-- View: combined_realtime — adds indoor_temperature_c, indoor_humidity_pct,
-- and davis_station_pressure_mb, all from the existing temp_humidity(davis)
-- join. Otherwise identical to migrations/20260703_add_station_roles.sql.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Event: evt_aggregate_history_charting — adds the same three indoor columns.
-- ─────────────────────────────────────────────────────────────────────────────
