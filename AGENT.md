...
- `esphome logs davis/davisnet-weatherlogger.yaml` for remote logs uses the native API — unlike the superseded build, `api:` is **active by default** here (needs `api_encryption_key` in `secrets.yaml`). Do **not** also add this node via Home Assistant's "ESPHome" integration UI, since HA entities come from `mqtt: discovery: true` instead, and having both would duplicate every entity.
